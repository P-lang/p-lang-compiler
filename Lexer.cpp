...
void Lexer::MakeToken(const string *source, TokenType tokenType, int startIndex, int length) {
    Token token;
...
void Lexer::Tokenize(const string *source) {
    ...
    while (true) {
        ...
        if (characterCategory == OPERATOR && state != COMMENT) {
            ...
            {
                MakeToken(source, TokenType::OPERATOR_LBRACE, currentIndex, 0);
            }
            else if(character == ')')
            {
                MakeToken(source, TokenType::OPERATOR_RBRACE, currentIndex, 0);
            }
        }
        currentIndex++;
    }
